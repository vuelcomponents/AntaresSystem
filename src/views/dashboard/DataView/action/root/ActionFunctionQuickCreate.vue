...
<div class="multiple-create" :key="life.counterKey">
  <div  :hidden="hide(param)" class="box" v-for="param in actionFunction.getInputs('text',null,true)" :key="param">
    <p class="form-label" >
      {{actionFunction.getHeaders([param],"first")}}
    </p>
    <InputText  :disabled="lock(param)" v-model="actionFunction[param]" class="form-text-input"/>
  </div>
  <div class="box" v-for="param in actionFunction.getInputs('numeric',null,true)" :key="param" :hidden="hide(param)">
    <p class="form-label">
  ...
  <div class="box" v-for="param in actionFunction.getInputs('dates',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
  ...
  <div class="box" v-for="param in actionFunction.getInputs('collections',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
  ...
  <div class="box" v-for="param in actionFunction.getInputs('objects',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
  ...
  <div v-for="param in actionFunction.getInputs('files',null,true)" :key="param" :hidden="hide(param)" style="display:flex;justify-content:center;flex-direction:column">
    <p class="form-label">
      {{actionFunction.getHeaders([param],"first")}} <span class="mini"> | Select </span>
    </p>
    <div>
  ...
  methods:{
    create(){
      this.crud.create(this.actionFunction).then(res=>{
        ...
        if(res?.status === 200) {
          if(res.data) this.emitter.emit('quickActionFunctionAdd', res.data)
        }
      })
    },
    ...
    hide(param){
      ...
      return this.actionFunction.$hide[param]({model:this.actionFunction})
    }
  },
  watch:{
    "actionFunction.systemFunction"(){
      this.life.counterKey++;
      this.actionFunction.realisations = []
    },

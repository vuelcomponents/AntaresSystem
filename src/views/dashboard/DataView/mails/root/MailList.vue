...
import MailMenu from "@/views/dashboard/DataView/mails/MailMenu";
...
import onCellClicked from "@/views/dashboard/DataView/mails/functions/onCellClicked";
...
export default {
  ...
  computed: {
    ...
    mounted(){
      this.emitter.on('quickMailAdd',(mail)=>{
        this.grid.api.applyTransaction({add:[mail]})
      })
    },
    data(){
        return{
            menu:MailMenu(this.createMail,
                              this.cancelChanges,
                              this.deleteSelected,
                              this.saveChanges,
                              this.loadList,
    ...
    methods:{
      ...
      quickAdd(){
        this.$route.name === 'MailList'
            ?
            this.$router.push({name: 'MailQuickCreate'})
            :

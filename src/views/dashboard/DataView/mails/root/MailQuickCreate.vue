...
<div class="multiple-create">
  <div :hidden="hide(param)" class="box" v-for="param in mail.getInputs('text',null,true)" :key="param">
    <p class="form-label">
      {{mail.getHeaders([param],"first")}}
    </p>
    <InputText  :disabled="lock(param)" v-model="mail[param]" class="form-text-input"/>
  </div>
  <div class="box" v-for="param in mail.getInputs('numeric',null,true)" :key="param" :hidden="hide(param)">
    <p class="form-label">
  ...
  <div class="box" v-for="param in mail.getInputs('dates',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
      {{mail.getHeaders([param],"first")}} <span class="mini"> | Date </span>
    </p>
    <Calendar :disabled="lock(param)" date-format="yy-mm-dd" v-model="mail[param]" class="form-text-input"/>
  </div>
  <div class="box" v-for="param in mail.getInputs('collections',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
  ...
  <div class="box" v-for="param in mail.getInputs('objects',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
  ...
  <div v-for="param in mail.getInputs('files',null,true)" :key="param" :hidden="hide(param)" style="display:flex;justify-content:center;flex-direction:column">
    <p class="form-label">
      {{mail.getHeaders([param],"first")}} <span class="mini"> | Select </span>
    </p>
    <div>
...
export default {
  ...
  setup(){
    ...
    const services = {
      emp: new MailService(emitter)
    };
    const toast = new Toast()
    const crud = new MailCrudController(services.emp, {toast})
    return {
      services,
      crud,
      toast,
  ...
  methods:{
    create(){
      this.crud.create(this.mail).then(res=>{
        ...
        if(res?.status === 200) {
          if(res.data) {
            this.emitter.emit('quickMailAdd', res.data)
          }
        }
      })
    },

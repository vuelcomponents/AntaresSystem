...
<div class="multiple-create">
  <div :hidden="hide(param)" class="box" v-for="param in company.getInputs('text',null,true)" :key="param">
    <p class="form-label">
      {{company.getHeaders([param],"first")}}
    </p>
    <InputText  :disabled="lock(param)" v-model="company[param]" class="form-text-input"/>
  </div>
  <div class="box" v-for="param in company.getInputs('numeric',null,true)" :key="param" :hidden="hide(param)">
    <p class="form-label">
  ...
  <div class="box" v-for="param in company.getInputs('dates',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
      {{company.getHeaders([param],"first")}} <span class="mini"> | Date </span>
    </p>
    <Calendar :disabled="lock(param)" date-format="yy-mm-dd" v-model="company[param]" class="form-text-input"/>
  </div>
  <div class="box" v-for="param in company.getInputs('collections',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
  ...
  <div class="box" v-for="param in company.getInputs('objects',null,true)" :hidden="hide(param)" :key="param">
    <p class="form-label">
  ...
  <div v-for="param in company.getInputs('files',null,true)" :key="param" :hidden="hide(param)" style="display:flex;justify-content:center;flex-direction:column">
    <p class="form-label">
      {{company.getHeaders([param],"first")}} <span class="mini"> | Select </span>
    </p>
    <div>
...
export default {
  ...
  setup(){
    ...
    const services = {
      emp: new CompanyService(emitter)
    };
    const toast = new Toast()
    const crud = new CompanyCrudController(services.emp, {toast})
    return {
      services,
      crud,
      toast,
  ...
  methods:{
    create(){
      this.crud.create(this.company).then(res=>{
        ...
        if(res?.status === 200) {
          if(res.data) {
            this.emitter.emit('quickCompanyAdd', res.data)
          }
        }
      })
    },

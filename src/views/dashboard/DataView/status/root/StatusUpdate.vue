  ...
  <div>
    <DataPopup :menuItems="popupOptions.menuItems" :stepperSettings="popupOptions.stepperSettings">
      <template v-slot:item1contentLeft>
        <div :hidden="hide(param)" class="box" v-for="param in status.getInputs('text')" :key="param">
          <p class="form-label">
      ...
      <template v-slot:item1contentRight>
        ...
        <div v-for="param in status.getInputs('colors')" :key="param" :hidden="hide(param)"
             style="padding-inline:5px;display:flex;justify-content:center;flex-direction:column">
          <p class="form-label">
...
export default {
  ...
  async mounted(){
    this.status = await this.crud.getById(this.$route.params.id)
  },
  methods:{
    update(){
      console.log('up', this.status)
      this.crud.update(this.status).then(res=>{
        if(res?.invalid){

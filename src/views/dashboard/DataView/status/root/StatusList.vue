...
import onCellClicked from "@/views/dashboard/DataView/status/functions/onCellClicked";
import StatusColumn from "@/views/dashboard/DataView/status/StatusColumn";
import StatusMenu from "@/views/dashboard/DataView/status/StatusMenu";
...
import EditOptions from "@/views/dashboard/DataView/status/options/EditOptions";
...
export default {
  ...
  computed: {
    ...
    setup(){
        ...
        const crud = new StatusCrudController(services.status)
    ...
    mounted(){
      this.emitter.on('quickStatusAdd',(status)=>{
        this.grid.api.applyTransaction({add:[status]})
      })
    },
    data(){
        return{
            menu:StatusMenu(this.createStatus,
                            this.cancelChanges,
                            this.deleteSelected,
                            this.saveChanges,
                            this.loadList,
    ...
    methods:{
        ...
        createStatus(){
          this.$router.push({name:'StatusCreate'})
        },
        deleteSelected(){
        ...
        quickAdd(){
          this.$route.name === 'StatusList'
              ?
              this.$router.push({name: 'StatusQuickCreate'})
              :

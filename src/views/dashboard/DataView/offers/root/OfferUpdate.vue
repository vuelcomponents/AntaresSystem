    ...
    <DataPopup :menuItems="popupOptions.menuItems" :stepperSettings="popupOptions.stepperSettings">
      ...
      <template v-slot:item1contentRight>
        ...
        <div  style="display:block">
          <div  :hidden="hide(param)" class="box" v-for="param in offer.getInputs('textAreaHeaders',null,true)" :key="param">
            <p class="form-label" >
              {{offer.getHeaders([param],"first")}}
            </p>
            <InputText  :disabled="lock(param)" v-model="offer[param]" class="form-text-input"/>
...
export default {
  ...
  async mounted(){
    this.offer = await this.crud.getById(this.$route.params.id)
  },
  methods:{
    update(){
      this.crud.update(this.offer).then(res=>{
        if(res?.invalid){
